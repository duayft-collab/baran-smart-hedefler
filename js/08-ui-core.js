...
var NAV=[
  ...
  {sec:'secProd',items:[{id:'goals',l:'Hedefler',i:'tgt'},{id:'goalsdash',l:'Hedef Panosu',i:'kpi'},{id:'okr',l:'OKR',i:'ref'},{id:'todos',l:'Yapılacaklar',i:'csq'},{id:'habits',l:'Alışkanlıklar',i:'star'},{id:'routines',l:'Rutinler',i:'ref'}]},
...
function closeModal(){
  ...
  if(typeof wqCaptureDraft==='function')wqCaptureDraft();
  ...
  window._pAfterSave=null; // P0-3: scrim/X ile kapatılırsa bekleyen decision→principle kancası sızmasın
  sh('modal-root','');
}
...
var FTITLES={goal:'Hedef',todo:'Görev',kpi:'KPI',habit:'Alışkanlık',journal:'Günlük',coaching:'Koçluk',vault:'Not',question:'Soru',sop:'SOP',routine:'Rutin',gtd:'GTD Notu'};
...
function submitForm(type){
  ...
  var km={goal:'goals',todo:'todos',kpi:'kpis',habit:'habits',journal:'journal',coaching:'coaching',vault:'vault',question:'questions'};
...
/* Kompakt Due Soon rozeti — metin destekli (renk yalniz vurgu), emoji YOK. Bos etiket → rozet yok.
   Done/future/deadline-yok → '' (rozet gösterilmez). Otomatik SIRALAMA yapmaz; salt bilgilendirici. */
function goalDueBadge(g){
  var lbl=(typeof goalDueLabel==='function')?goalDueLabel(g):'';
  if(!lbl)return '';
  var st=(typeof goalDueState==='function')?goalDueState(g):'none';
  var col={overdue:'var(--red)',today:'var(--red)',tomorrow:'var(--orange)',this_week:'var(--orange)',due_soon:'var(--blue)'}[st]||'var(--t2)';
...
function renderGoals(){
  ...
  if(gView==='list'){
    ...
    goals.forEach(function(g){
      ...
      h+='<td style="padding-left:16px"><div style="display:flex;align-items:center;gap:7px">'+(g.frog?'<span style="font-size:14px">&#128056;</span>':'')+'<div><p style="font-weight:600;font-size:13px">'+U.esc(g.title)+'</p>'+(g.measurable?'<p style="font-size:10.5px;color:var(--t2)">'+U.esc(g.measurable)+'</p>':'')+'<div style="display:flex;gap:5px;align-items:center;margin-top:3px">'+goalHealthBadge(g)+goalPriorityBadge(g)+goalDueBadge(g)+((typeof goalDependencyBadge==='function')?goalDependencyBadge(g):'')+'</div></div></div></td>';
    ...
    goals.forEach(function(g){
      ...
      h+='<p style="font-weight:700;font-size:14px;line-height:1.3">'+U.esc(g.title)+'</p><div style="display:flex;gap:5px;align-items:center;margin-top:4px">'+goalHealthBadge(g)+goalPriorityBadge(g)+goalDueBadge(g)+((typeof goalDependencyBadge==='function')?goalDependencyBadge(g):'')+'</div></div>';
