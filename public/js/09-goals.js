function openGoalDetail(goalId){
  ...
  var dueLbl=(typeof goalDueLabel==='function')?goalDueLabel(g):'';
  h+='<div style="display:flex;gap:5px;margin-top:4px;flex-wrap:wrap"><span style="'+hpill+'">'+U.esc(g.cat||'')+'</span><span style="'+hpill+'">'+goalPlanningLabel(g)+'</span><span style="'+hpill+'">'+goalPriorityLabel(g)+'</span><span style="'+hpill+'">'+goalHealthLabel(g)+'</span><span style="'+hpill+'">Güven: '+goalConfidenceLabel(g)+'</span>'+(dueLbl?'<span style="'+hpill+'">'+dueLbl+'</span>':'')+'</div></div></div>';
  ...
  h+='<div id="goal_wisdom_box">'+((typeof wiCardHtml==='function')?wiCardHtml(wiCtxFromGoal(g)):'')+'</div>'; // CROSS-K1: bağlamsal İlgili Bilgelik (türetilmiş, salt-okunur, 0 write)
